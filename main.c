...
uint16_t arraySize = sizeof(patterns) / sizeof(patterns[0]);
...
int main(void)
{
  ...
  {
	  //For Debugin the Data being written
	  //GPIOB->ODR = patterns[i];
	  delay(200000);
      write_to_address(i, patterns[i]);  // i is the address, myArray[i] is the data to write
  }
  /* Infinite loop */
...
void TIM16_IRQHandler(void)
{
  ...
  if (value != patterns[currentAddress]){
    ...
    {
        currentAddress = 0;  // Reset to start address if end of EEPROM is reached
    }
}
